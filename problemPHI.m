...
function A = problemPHI ( nodes,elements,sigma,epsilon,phi,alpha,beta )
...
for k = 1:size(elements,1)
    ...
    for i = 1:numPhaseFields 
        vals = vol*beta(i)*epsilon*BinvDQuad{k};
        valsSPhi(counter:counter+8,i) = vals(:);
        vals = [2,1,1;1,2,1;1,1,2] * (vol/12) * ( sigmaNrmSqr/epsilon*(region==i) + beta(i)/epsilon );
