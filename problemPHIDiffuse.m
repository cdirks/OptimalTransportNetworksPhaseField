...
function A = problemPHIDiffuse ( nodes,elements,sigma,epsilon,phi,alpha0,alpha,beta )
...
for k = 1:size(elements,1)
    ...
    sigmaVals = sigma(k,:);
    sigmaNrm = sqrt(sigmaVals(:,1).^2+sigmaVals(:,2).^2);
    ...
    if ( sigmaNrm > epsilon*alpha0/(2*min(phiValsSqrEta)) )
        region = 0; 
    end
